...
print("Final values")
print("----------------------------")
print("total charge at the end (nC): ", q_sn + q_se + q_sg + q_dn + q_de + q_dg)
print("Q_sn + Q_sg (nC):", q_sn+q_sg)
print("Q_se (nC): ", q_se)
print("Q_dn + Q_sg (nC):", q_dn+q_dg)
print("Q_de (C): ", q_de)
print('total volume (cm^3):', my_cell.V_sn[-1] + my_cell.V_se[-1] + my_cell.V_sg[-1] + my_cell.V_dn[-1] + my_cell.V_de[-1] + my_cell.V_dg[-1])
print("----------------------------")
print('elapsed time: ', round(time.time() - start_time, 1), 'seconds')

plt.figure(1)
plt.plot(t*1e-3, phi_msn, '-', label='soma')
plt.plot(t*1e-3, phi_mdn, '-', label='dendrite')
plt.title('Neuronal membrane potentials')
plt.xlabel('time [s]')
plt.legend(loc='upper right')

plt.figure(100)
plt.plot(t*1e-3, phi_msg, '-', label='somatic layar')
plt.plot(t*1e-3, phi_mdg, '-', label='dendritic layer')
